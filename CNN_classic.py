...
def conv_net(_X,_width_input,_height_input,_weights,_strides,_biases,_dropout):
    ...
    lay1_actv=tf.nn.relu(tf.nn.bias_add(lay1_conv,_biases['bc1']))
    lay1_pool=max_pooling(lay1_actv, k=2)
    lay1_drop=tf.nn.dropout(lay1_pool,keep_prob=_dropout)
    lay1_output=lay1_drop
    
    
    #Layer 2
    lay2_input=lay1_output
    lay2_conv=tf.nn.conv2d(input=lay2_input,filter=_weights['wc2'],strides=_strides['sc2'],padding='SAME')
    lay2_actv=tf.nn.relu(tf.nn.bias_add(lay2_conv,_biases['bc2']))
    lay2_pool=max_pooling(lay2_actv, k=2)
    lay2_drop=tf.nn.dropout(lay2_pool,keep_prob=_dropout) 
    lay2_output=lay2_drop   
    ...
    lay3_actv=tf.nn.relu(tf.add(lay3_fccd,_biases['bd1']))
    lay3_drop=tf.nn.dropout(lay3_actv,_dropout)
    lay3_output=lay3_drop
    
    
    #lay4: output
    lay4_input=lay3_output
    lay4_output = tf.add(tf.matmul(lay4_input,_weights['out']),_biases['out'])
